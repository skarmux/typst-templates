...
#table(
  columns: (1fr, auto), align: (horizon + center, horizon + center), stroke: none, stack(
    dir: ttb, spacing: 1em, keyword(
  ...
  box[#grid(
      ...
      align(
        center,
          ...
          dir: ttb, spacing: 1em, align(center)[#job.span.end], align(center)[󱇻], align(center)[#job.span.begin],
        )], box(width: 1fr),
      // 2. Column: content
      box(
        width: 1fr,
        ...
        #stack(
          dir: ltr, spacing: 1em, ..job.highlights.map(
            tool => rect(
              inset: 0.75em, stroke: 1.5pt + gradient.linear(colors.complement, colors.primary, angle: 45deg), height: 2em, radius: 50%,
            )[
        ...
        #line(
          length: 100%, stroke: 1pt + gradient.linear(colors.base, colors.complement, colors.base),
        )
        #box(width: 1fr)[
...
#columns(
  3, gutter: 1em, for (index, pair) in glossary.pairs().enumerate() {
    ...
    box(
      stack(
        dir: ttb, spacing: 0.5em, align(center, text(weight: "bold", headers.at(category).at("de"))), v(0.5em), rect(
          ...
          .map(
            elem => {
              ...
              stack(
                spacing: 0.5em, align(
                  left, text(weight: "bold", fill: colors.primary, headers.at(elem.at(0)).at("de")),
...
#box(
  ...
  #for cert in data.certificates [
    #grid(
      columns: (-2.5cm, 2.5cm, 1fr), box(width: 1fr), h(1fr), [*#cert.title*\ #cert.location], v(0em),
    )
  ]
]
#v(1.5em)
...
#align(
  center, stack(
    dir: ltr, spacing: 1em, ..data.interests.map(
      a => align(
        horizon, rect(
          inset: 0.75em, stroke: 1.5pt + gradient.linear(colors.complement, colors.primary, angle: 45deg), height: 2em, radius: 50%,
        )[#a],
